	...
	return (
		<div
			className={`${clsx({
				...
				"bg-neutral-100 bg-opacity-20 border-b backdrop-blur": simpified === true,
			...
			<div className="flex items-center justify-between">
				...
				<div className="w-96 flex justify-evenly">
					<Delimiter className="hidden lg:block" variant="line" height="medium" />
					{pages.map((page) => {
						return (
							<Link
								href={`/${page.toLowerCase()}`}
								key={page}
								className="transition-all hidden lg:flex overflow-x-hidden underline-offset-4 decoration-gray-500 hover:text-gray-500 hover:underline focus:underline focus:decoration-violet-600 outline-none">
								{page}
							</Link>
						);
					})}
